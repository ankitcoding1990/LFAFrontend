...
export class FunnelComponent implements OnInit {
	...
	prev_section(data: any) {
		...
		if (data.target.value == "have_legal_again") {
			...
			this.have_legal_no();
			if (sessionStorage.getItem('legal_id') === null || sessionStorage.getItem('legal_id') === 'null' || sessionStorage.getItem('legal_id') == '') {
				this.show_address_form = true;
				this.nav_value = "ADDRESS";
				this.main_nav = "PROFILE";
				this.show_legal_form = false;
		...
		if (data.target.value == "legal_issue") {
			...
			//this.legal_issue_matter();
			if (sessionStorage.getItem('legalissue_id') != null || sessionStorage.getItem('legalissue_id') != '' || sessionStorage.getItem('legalissue_id') != '0') {
				this.legalform.get("legal_issue").setValue(sessionStorage.getItem('legalissue_id'));
			}
		}
	...
	get_user_basic_info() {
		...
		this.dataService.getuserdetails(this.email).subscribe((data: {}) => {
			...
			if (this.userdetails.result.legal_metter != null) {
	...
	save_address() {
		this.dataService.update_user_funnel(this.Addressform).subscribe(
			(response) => {
				...
				if (sessionStorage.getItem('legal_id') === null || sessionStorage.getItem('legal_id') === "null" || sessionStorage.getItem('legal_id') == '') {
					this.show_legal_form = true;
					//console.log('legal id',sessionStorage.getItem('legal_id'));	
					this.show_legal_dropdown();
				} else {
	...
	save_custom_legalform() {
		Swal.fire({
			...
			html: 'We understand this process can be complicated and isnâ€™t the most delightful matter to talk about. Rest assured, youâ€™re doing the right thing! <br> <br>Letâ€™s keep getting your account set up.',
		})
	...
	show_legal_dropdownwithno()
	{
		this.legalform.get("legal_type").patchValue('');
		this.legalform.get("legal_issue").patchValue('');
		this.legalform.get("legal_issue_description").patchValue('');
		this.show_legal_dropdown();
	}
	show_legal_dropdown() {
		...
		this.legal_matter_dropdoen = true
		this.nav_value = "LEGAL MATTER";
		//sessionStorage.setItem('legalissue_id', '0');
		//console.log('aaaa','show_legal_dropdown');
	...
	show_legal_issue(data: any) {
		...
		if (data.target.value == "with_legal_matter") {
			this.prev_buton_legal = true;
			if (sessionStorage.getItem('legalissuehide') != null) {
				this.legal_issue_dropdoen = false;
				...
				this.title_legal = false;
				this.show_legal_description();
				sessionStorage.removeItem('legalissuehide');
				$("#prev_from_desc").val("legal_matter");
				//console.log('1073')
			} else {
				this.legalform.get("legal_issue").setValue('');
				this.legal_issue_matter();
				$("#prev_from_desc").val("legal_issue");
				//console.log('1078')
			} 
	...
	legal_issue_matter() {
		if (sessionStorage.getItem('legalissuehide') != 'now') {
			this.main_nav = "LEGAL DETAILS";
			this.legal_description = false;
	...
	getotherlegalinterests() {
		...
		this.dataService.get_othet_instdetail().subscribe((data: {}) => {
			...
			if (sessionStorage.getItem('legal_ide') == null) {
				this.legalform.get("legal_type").setValue("");
			}
		});
	...
	onChange_other_interests(event: any) {
		...
		this.dataService.get_sub_legal_interests(event).subscribe(
			(response) => {
				sessionStorage.setItem('legal_id', event);
				this.get_sub_interests = response.data;
				//console.log(this.get_sub_interests)
				if (response.display == "none") {
					this.legal_issue_show = false;
					sessionStorage.setItem('legalissuehide', 'now');
				} else {
	...
	onChange_other_issueform(event: any) {
		sessionStorage.setItem('legalissue_id', event);
